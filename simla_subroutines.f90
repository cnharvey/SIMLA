!---------------------------------------------------------------
! SIMLA SUBROUTINES
! Copyright C.N. Harvey and D.G. Green 2014
!---------------------------------------------------------------
...
do 
	...
	if (iostatvalue .lt. 0) then 	
		exit 	
	end if
	if (iostatvalue .gt. 0) then 
...
! Display on the screen how many output fields created
print '  (3x,I4.4, " field data files created")',fieldpointst
...
!  mm = month and dd = day
   ...
   write(*,*) '|                     RUN ON ', Day,'/',Month,'/', Year,' at ', PrettyTime,  '                       |'
